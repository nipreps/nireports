...
"""Plotting no necessarily distributions."""
...
def _compute_crop_slices(img: nb.spatialimages.SpatialImage) -> tuple[slice, slice, slice] | None:
    ...
    # Guarantee exactly 3 slices (x, y, z)
    return (
...
def _merge_crop_slices(
    ...
    t0, t1, t2 = second
    return (
        slice(min(s0.start or 0, t0.start or 0), max(s0.stop or 0, t0.stop or 0)),
        slice(min(s1.start or 0, t1.start or 0), max(s1.stop or 0, t1.stop or 0)),
        slice(min(s2.start or 0, t2.start or 0), max(s2.stop or 0, t2.stop or 0)),
    )
...
def plot_motion(
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        for idx, frame in enumerate(frames):
            ...
            png_bytes = imageio.imwrite("<bytes>", frame, format="PNG")
            data_uri = b64encode(png_bytes).decode("ascii")
            svg_parts.append(
